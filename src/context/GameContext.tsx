...
export function GameProvider({ children, startFresh = false, initialState: injectedState }: { children: React.ReactNode; startFresh?: boolean; initialState?: GameState }) {
  // Start with a default state, we'll load from localStorage after mount (unless startFresh is true)
  const [state, setState] = useState<GameState>(() => injectedState ?? createInitialGameState(DEFAULT_GRID_SIZE, 'IsoCity'));
  ...
  useEffect(() => {
    ...
    // Load game state (unless startFresh is true or injectedState was provided)
    if (injectedState) {
      // injectedState already set via useState initializer — skip localStorage
