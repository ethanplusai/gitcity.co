...
export async function sourceLayout(store, id, files, inventory, ref) {
  const result = await store.directoryInventory(
    id,
  ...
  const active = Boolean(
    (inventory && ref) ||
    (await store.db
      .prepare('SELECT 1 FROM directory_land WHERE repo=? LIMIT 1')
      .get(id.toLowerCase())),
  );
  const { files: surveyed, ...summary } = result;
  if (active) summary.directories = await store.inventoryLand(id, summary.directories);
  const located = active ? await store.inventoryFiles(id, surveyed) : surveyed;
  const sources = await store.sourceLand(
    id,
